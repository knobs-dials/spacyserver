...
def force_color():
    ' tell ourselves to output color in any context '
    guess_color_support(True, True, True)

def no_color():
    ' tell ourselves to output color in no contexts '
    global _guess
    _guess = None
...
def supported():
    ' return the result of guess_color_support, cached once it has been run before '
    global _guess
    _guess = None
    if _guess is None:
        guess_color_support()
...
def tty_size(debug=False):
    """ fetches current terminal size
        ...
        at the least something like   `sc.tty_size()['cols'] or 80` but probably a bit more complex
    """
    ...
    # maybe make this conditional on detecting windows in the first place?
    try:
    ...
    # probably we want more specific exceptions
    except Exception as exc: # pylint: disable=broad-exception-caught
        if debug:
            raise exc
    if ret['rows'] not in (0,None) and ret['cols'] not in (0,None):
...
def brightblack(s, prepend=''):
    ' return prepend+BRIGHTBLACK+s+RESET or, if no support, just s '
...
def brightgrey(s, prepend=''):
    ' return prepend+BRIGHTGREY+s+RESET or, if no support, just s '
...
def _strip_escapes_if_not_supported(s, forceaway=False):
    if _guess and not forceaway:
        ...
        return _strip_escapes(s)

def _add_color_if_supported(s, colcode, prepend=''):
    ' returns prepend+colcode+s+RESET  or, if we guessed no color support, just s' 
    if _guess:
...
def truncate_real_len(s,newlen, append=RESET):
...
def color_degree(s, v, fromv=0, tov=0, colors=(BRIGHTBLACK, GRAY, WHITE, YELLOW, RED)):
    ''' I find the need for color as an indicator for amount/degree of something.
        This helps do that -- within the minimal color set.
    
        We take values in the range fromv...tov
         - <= fromv is the first color
         - >= tov is the last
         - between is interpolated
    
        TODO: check this works if tov<fromv
    '''
    # hacky
    v = float(v)
    tov = float(tov)
...
def blend(s, frac:float, rgb1, rgb2):
    ' blend (with a 0..1 fraction) between two (r,g,b) tuples with a and put the result in a truecolor escape '
...
def color_urlpath(s,
                  sep='/',
                  dirname_col  = darkgray,
                  basename_col = white,
                  query_col    = darkgray,
                  query_params = None,
                  #CONSIDER: add colors for all other parts
                  #CONSIDER: make each optional?
                  # proto_col, host_col, port_col,
                  fragment_col = darkgray,
                  remove_fragment=True
    ):
    ''' Take an URL, and print the path's dirname, path's basename, and query in separate colors
        e.g. given a/b/c/d?e=f will color
        - 'a/b/c' using dirname_col
        - '/d'    using basename_col
        - '?e=f'  using query_col
        
        - query_params is intended to selectively color some parameters, e.g. query_params={'e':sc.blue} would color the 'e=f' in the example blue 

    '''
    po = urllib.parse.urlparse(s)
    path     = po.path
    query    = po.query
    dirname, basename = path.rsplit( sep, 1 )
    ret = '/'.join( [dirname_col(dirname), basename_col(basename)] )
    if len(query) > 0:
        if query_params is not None  and  isinstance(query_params, dict):
            nq = []
            for part in query.split('&'):
                changed = False
                for teststr, colf in query_params.items():
                    if part.startswith('%s='%teststr):
                        nq.append( colf(part) )
                        changed = True
                        break
                if not changed:
                    nq.append( query_col(part) ) # TODO: think
        ret += '?'+'&'.join(nq)
    if not remove_fragment  and  len(po.fragment)>0:
        ret += fragment_col( '#'+po.fragment )
    return ret
